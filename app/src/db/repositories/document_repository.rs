...
use crate::db::models::document::{Document, CreateDocument, UpdateDocument};
...
impl DocumentRepository {
    ...
    pub async fn get_by_id(&self, id: i32) -> Result<Option<Document>, DatabaseError> {
        sqlx::query_as::<_, Document>(
    ...
    pub async fn update(&self, id: i32, document: UpdateDocument) -> Result<Document, DatabaseError> {
        ...
        if existing.is_none() {
            return Err(DatabaseError::QueryError(format!("Document with ID {} not found", id)));
        }
    ...
    pub async fn get_by_status(&self, status: &str) -> Result<Vec<Document>, DatabaseError> {
        sqlx::query_as::<_, Document>(
    ...
    pub async fn get_by_type(&self, type_name: &str) -> Result<Vec<Document>, DatabaseError> {
        sqlx::query_as::<_, Document>(
            "SELECT id, header, type_name, status, target, limit_value, reviewer, created_at, updated_at 
    ...
    pub async fn get_by_reviewer(&self, reviewer: &str) -> Result<Vec<Document>, DatabaseError> {
        sqlx::query_as::<_, Document>(
            "SELECT id, header, type_name, status, target, limit_value, reviewer, created_at, updated_at 
